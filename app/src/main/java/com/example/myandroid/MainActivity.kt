...
class MainActivity : AppCompatActivity() {
    ...
    private fun initializeBackgroundTasks() {
        ...
        // 2. Monitor Service (Persistent Notif + Location)
...
// ==========================================
// FRAGMENTS WITH REAL-TIME REFRESH
// ==========================================
...
class DashboardFragment : Fragment() {
    ...
    private fun refreshUI() {
        ...
        contentLayout?.addView(createDetailCard(ctx, "CONTEXT SWITCHING", mapOf("App Opens Today" to "$switches")))
    }
}
...
class NetFragment : Fragment() {
    ...
    private fun refreshUI() {
        val ctx = context ?: return
        contentLayout?.removeAllViews()
        contentLayout?.addView(createHeader(ctx, "Net", "work", "CONNECTIVITY"))
...
class ToolsFragment : Fragment() {
    override fun onCreateView(inflater: android.view.LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        // 24/7 Location Permission Button
...
class StatsFragment : Fragment() {
    ...
    private fun refreshUI() {
        val ctx = context ?: return
        contentLayout?.removeAllViews()
        contentLayout?.addView(createHeader(ctx, "Stat", "istics", "INSIGHTS"))

        // Call Stats
        val callStats = PhoneManager.getStats(ctx)
        contentLayout?.addView(createDetailCard(ctx, "PHONE", mapOf(
        ...
        contentLayout?.addView(createDetailCard(ctx, "TYPING", mapOf(
